...
#### 5. 线程的切换
  ...
  在做 Task 5 时，身边一些同学认为将多线程实现中子线程的 TCB 当作 PCB 一样挂在 PCB 的队列中即可。然而我认为这样不好，线程并不是进程，同一个进程的多个线程不该被当作多个进程来对待，虽然这样能够通过 Task 5。由于我打算将多线程功能在后面的实验项目中保留下去，所以我还是选择了在每个进程的 PCB 下方挂接了由其子线程 TCB 组成的单链表这样的方法，并且 TCB 是单独设计的数据结构，它仅有上下文`context`、进入时的参数`arg`、TID`tid`、栈基址`stack`、指向链表下一个结点的指针`next`这几个成员。
...
```
  ...
  尽管坚持了“理清思路、打好草稿再动手”的原则，这次我还是翻车了。线程调度一上来就出了问题：在这种设计中，同一个进程的多个线程是共享一个 PCB 的，进而也共享一个`trapframe`（发生异常时保存的上下文）。而我们线程的切换`thread_yield()`是以系统调用的方式执行的，如果我们简单粗暴地使用`switch_to()`类似的函数直接做上下文切换然后`sret`返回，那么由于当主线程做该系统调用时就要执行`ecall`指令，而子线程做该系统调用时也要`ecall`，就会导致主线程`ecall`时保存在`trapframe`中的上下文被冲掉了，产生了严重的错误。

  翻车了！我开始不知所措，终于经过思索，我在原有方案的基础上对线程切换的方法做了如下修改：当线程切换（某个线程调用了`thread_yield()`）时，我们把当前`trapframe`中所有保留寄存器（Callee saved registers）以及`sepc`保存到当前线程的`context`中，再把下一个线程相关的信息恢复到`trapframe`中，然后直接进行异常返回，就切到了下一个线程。实话实说，仅把保留寄存器和`sepc`转存到`context`是不安全的，设想一种极端的情况：某个线程的函数中直接用内联汇编的方式调用了`thread_yield()`，这样当线程调度再次切换回它时，它的非保留寄存器（Caller saved registers）就可能变了。因此，要想保证正确，就需要在子线程中使用调用完整 C 语言函数（`invoke_syscall()`）的方法来做`thread_yield()`，这样在函数调用时，编译器会为我们做非保留寄存器的保存工作。

#### 6. 线程的进入

  由于我想保留多线程的功能，所以我还实现了线程的终止（`thread_kill()`），主线程可进行该系统调用将指定 TID 的线程结束。想要杀掉子线程，当然要使用我自己的`ufree_g()`释放为其分配的栈空间。然而在释放栈空间时，整个系统直接死掉了。

  启动 GDB 大法，经过检查，我发现系统死掉的原因是在调用存储释放函数`ufree_g()`时，本线程的栈空间所用的主存块的 Tag 的数据错了（注：存储分配我使用的是边界标识法，即 Boundary Tag，需要在每一个块的两端添加一块标记）。怎么回事，是 Stack Overflow（因为刚遇到过一次这种错误，因此比较敏感）吗？如果另一个线程的栈与之相邻而它溢出了，就可能涂改掉这个线程的栈空间的 Tag。于是我增加了线程的栈的大小，并没有解决问题，但我注意到增加了每个线程栈空间大小后，Tag 出错的数据与之前一样，这很可能意味着 Tag 是被这个线程自己涂改掉的。

  在我疑惑时我突然想到，线程切换用的是系统调用的方式。在异常返回`ret_from_exception()`时，对系统调用异常会做一个特殊的处理，即将`sepc`自增 4 再`sret`。而在子线程首次运行时，我是直接将子线程的入口地址填到了`trapframe`的`sepc`上。这就导致首次进入子线程时会因为对`sepc`自增 4 而跳过至少第一条指令，那么子线程函数（`v0_inc()`）的第一条指令是什么呢？看一下反汇编……

```assembly
00000000520001b4 <v0_inc>:
    520001b4:	1101                    addi    sp,sp,-32
    520001b6:	ec06                    sd      ra,24(sp)
    ...
```

  破案了，这个 Bug 导致完美地跳过了第一条将`sp`降低（分配栈帧空间）的指令，进而导致后面对栈帧的操作会写越界（写到比初始`sp`更高的地方去），涂改掉了栈空间的 Tag。

  解决方法，自然是在初始化新生线程时，将入口地址减去 4 再去改写`trapframe`的`sepc`。
