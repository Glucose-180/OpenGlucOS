...
#### [2023-12-30]
  ...
  **热烈祝贺北京城市轨道交通 17 号线北段、16 号线剩余段和 11 号线西段剩余段今日开通运营！**
...
#### [2023-12-31]
  ...
  又取消了一部分`volatile`限定符（纯必要，感觉没路人）。新增文件系统缓存（GFS Cache）功能。目前初步测试似乎没问题，尚未进行高强度测试。
...
#### [2024-01-01]
  ...
  **新年快乐！**
  ...
  发现一个问题：在对空文件`tp3`执行`foptest tp3 -l9437184 -n200 &`时可能由于调用`screen_write()`打印出缓冲区内的无效字符导致屏幕出问题，甚至无法操作。

  修复了`foptest.c`中打印的操作，根据实际读到的字符数进行打印。修改了`rwfile2.c`，新增计时功能（计量文件读写操作所耗费的时间）以测试缓存效果，使 rwfile2 测试程序能接受`-m`参数从而一次进行 MiB 单位的读写。目前 -O2 上板比较正常

  书写了`README.md`，本学期的原神实验还差一些尾巴就可以宣告结束了。

#### [2024-01-02]

  进行 Project 6 的检查，也是本实验课的最后一次检查了。
